...
static void write_connect(connection_map &conns, int kq, io_handle listen_fd, io_handle client_fd) {
...
static void write_back(connection_map &conns, io_handle listen_fd, int client_fd, int kq) {
    auto it = conns.find(client_fd);
    if (it == conns.end()) {
        return;
    }
...
static void create_connect(connection_map &conns, io_handle listen_fd, int kq) {
    while (true) {
...
static void read_connect(connection_map &conns, io_handle listen_fd, io_handle client_fd, int kq, auto &on_recv, io* io) {
    ...
    while (true) {
        ...
        if (received) { // 读取完毕，回调
            ...
            write_back(conns, listen_fd, client_fd, kq);
        }
    }
}
    ...
    while (true) {
        ...
        for (int i = 0; i < res_size; i++) { // 遍历所有事件
            ...
            if (event.flags & EV_ERROR) { // 错误
                ...
                create_connect(connections, listen_fd, kq);
            } else if (event.filter == EVFILT_READ) { // 客户端发来数据
                read_connect(connections, listen_fd, fd, kq, on_recv, this);
            } else if (event.filter == EVFILT_WRITE) { // 客户端可写
                write_connect(connections, kq, listen_fd, fd);
            } else {
                // TODO: 未完成
